    ...
    <header>
		...
		<script>
			function qq(){
				...
				show_apphistory('adaptation_scale_pw.php?n='+s1);
			}
		</script>
	</header>
	<body>
		...
		<table align='center' width='50%'>
			...
			<?php
				if(isset($_GET['n'])){
					$n=($_GET['n']*10)-10;
					$sel_adaptation=mysqli_query($conn,"SELECT * FROM `adaptation_scale_w` where `parent_id`<>\"\" ORDER BY `parent_id`,`write_time` limit ".$n.",10");
				}else{
					$sel_adaptation=mysqli_query($conn,"SELECT * FROM `adaptation_scale_w` where `parent_id`<>\"\" ORDER BY `parent_id`,`write_time` limit 0,10");
				}
		...
		<?php
			//查詢資料筆數
			$sel_number=mysqli_query($conn,"select * from `adaptation_scale_w` where `parent_id`<>\"\"");
