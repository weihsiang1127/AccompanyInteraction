    ...
    <header>
		...
		<script>
			function qq(){
				...
				if(stu==""){
					show_apphistory('interactive_scale_w.php?n='+s1);
				}else if(stu!=""){
					show_apphistory('interactive_scale_w.php?stu='+stu+"&n="+s1);
				}*/
				show_apphistory('interactive_scale_w.php?n='+s1);
			}
		</script>
	...
	<body>
		...
		<table align='center' width='50%'>
			...
			<?php
				if(isset($_GET['n'])){
					$n=($_GET['n']*10)-10;
					$sel_interactive_scale_w=mysqli_query($conn,"SELECT * FROM `interactive_scale_w` ORDER BY `parent_id`,`write_time` limit ".$n.",10");
				}else{
					$sel_interactive_scale_w=mysqli_query($conn,"SELECT * FROM `interactive_scale_w` ORDER BY `parent_id`,`write_time` limit 0,10");
				}
		...
		<?php
			//查詢資料筆數
			$sel_number=mysqli_query($conn,"select * from `interactive_scale_w` where `parent_id`<>\"\"");
